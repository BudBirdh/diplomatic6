...
ros2 run openvslam_ros run_slam --rectify --eval-log -v orb_vocab.fbow -c /opt/openvslam/openvslam/example/euroc/EuRoC_stereo.yaml --map-db map.msg --ros-args -p use_sim_time:=True -r /camera/left/image_raw:=/cam0/image_raw/throttled -r /imu/filter_data:=/imu/filter_data -r /camera/right/image_raw:=/cam1/image_raw/throttled -p publish_tf:=False -r /openvslam_odom:=/openvslam_odom -p map_frame:=odom -p base_link:=cam0 &
ros_id=$!
sleep 3;
cd ~/
echo "trying bag"
ros2 bag play MH_04_throttled_5hz --clock 1000 &
bag_id=$!
cd /opt/slam_ws/src/openvslam_ros
while true;do 
